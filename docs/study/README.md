...
칼만 필터(Kalman Filter)의 동작 원리를 깊이 있게 이해하기 위해, 그 근간이 되는 세 가지 재귀적(Recursive) 필터의 학습 기록입니다.

---

## 1. 필터 비교 요약
데이터를 쌓아두고 계산하는 배치식이 아닌, 이전 결과값에 새 데이터를 더하는 재귀식으로 구현되었습니다. 이는 메모리 사용량을 최소화해야 하는 실시간 시스템(레이더, 자율주행 등)에 필수적인 구조입니다.
